...
from .models import *
from .forms import CreateUserForm, signUpClassForm
...
def student_home(request):
    ...
    pastProgressions = StudentCurriculum.objects.filter(
        studentid=request.user, statusid=3)
    currentProgressions = StudentCurriculum.objects.filter(
        studentid=request.user, statusid__in=[1, 2])
    context = {"pastProgressions": pastProgressions,
               "currentProgressions": currentProgressions}
...
def display_curriculum_challenges(request, curriculumid):
    challengeList = ChallengeCurriculum.objects.filter(
...
def instructStudentChallenge(request):
    ...
    curriculum = Curriculum.objects.get(id=curriculumid)
    student = User.objects.get(id=studentid)
    sce_list = StudentChallengeEvent.objects.filter(
        studentid=studentid, curriculumid=curriculumid, progressionid=curriculum.progressionid)
    context = {"student": student, "curriculum": curriculum,
               "challenge": challenge, "sce_list": sce_list}
...
def instructStudentChallenge_Submit(request):
    ...
    if recording is not None:
        ...
        path = default_storage.save(
            videofilepath, ContentFile(recording.read()))
        if (request.POST.get('resultCode') == "pass"):
            StudentChallengeEvent.objects.create(progressionid=progression, studentid=student, curriculumid=curriculum, challengeid=challenge,
                                                 instructorid=request.user, resultcode=True, comment=assessment_comments, videofile=videofilepath)
        else:
            StudentChallengeEvent.objects.create(progressionid=progression, studentid=student, curriculumid=curriculum, challengeid=challenge,
                                                 instructorid=request.user, resultcode=False, comment=assessment_comments, videofile=videofilepath)
    else:
        if (request.POST.get('resultCode') == "pass"):
            StudentChallengeEvent.objects.create(progressionid=progression, studentid=student, curriculumid=curriculum,
                                                 challengeid=challenge, instructorid=request.user, resultcode=True, comment=assessment_comments)
        else:
            StudentChallengeEvent.objects.create(progressionid=progression, studentid=student, curriculumid=curriculum,
                                                 challengeid=challenge, instructorid=request.user, resultcode=False, comment=assessment_comments)
    ...
    query_string = urlencode(
        {'student': studentid, 'curriculum': curriculumid})  # 2 category=42
